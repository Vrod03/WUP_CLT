...
In the beginning we had a bi-modal distribution graph. Along with where the graph progress down into uni-modal. This graph is uni-modal where the 9 samples are.
...
As the n value gets bigger the graph increasingly becomes more uni-modal. This is when the 16 samples are averaged together. 
...
Now with 25 samples averaged together the graphs peak is completely uni-modal in comparison to the graph with smaller sample sizes.
...
By going from a sample size of 9 to 36 we were able to further analysis the Central Limit Theorem. We have demonstrated that the larger the sample size became the more the graphs transitioned into uni-modal. With this sample size of 36 the graphs averaged together with a close to perfect even distribution.
